<template>
  <div id="klinemain"></div>
</template>
<script>
import * as echarts from "echarts";
import moment from 'moment';
import jsonData from '../../data/2023-11-10/SH600000.json'
export default {
  data() {
    return {
      msg: "hello",
  ...
  mounted() {
    this.formatterList(jsonData.item)
    this.init();
  },
  methods: {
    // [日期，开盘价，收盘价，均价，涨幅，最低价，最高价，]
    formatterList(list) {
      console.log(list);
      return list.map(item => {
        let res = []
        res[0] = moment(item[0]).format('YYYY/MM/DD'); // date
        res[1] = item[2]; // open
        res[2] = item[5]; // close
    ...
    init() {
      ...
      let myChart = echarts.init(document.getElementById("klinemain"), null, {
